...
import PhilospyCard from './PhilospyCard';



const Philospy = () => {
    const [philospy,setphilospy] = useState([])
    useEffect(()=>{
        fetch('Philosophy.json')
        .then(res=> res.json())
        .then(data=> setphilospy(data))
    },[])
    ...
    return (
        <div>
          ...
          '>
                {
                    philospy.map(philospy=><PhilospyCard
                         key={philospy.id}
                         philospy={philospy}>
                         </PhilospyCard>)
                }
